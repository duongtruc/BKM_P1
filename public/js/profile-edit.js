...
$(function () {
    ...
    $("sup").click(function () {
        $(this).parent().children(".profile-detail  .profile-content").each(function () {
            $(this).html('<input onkeypress="userUpdateKeyPress($(this).parent().attr(\'id\'), , event.keyCode)"  required="true" type="text" size = "'
                + 2 * $(this).text().length + '" autofocus="true" value = "'
                + $(this).text() + '">');
        });
        $(this).parent().children(".profile-detail  .profile-content-shortDes").each(function () {
            $(this).html('<textarea onkeypress="userUpdateKeyPress($(this).parent().attr(\'id\'), event.keyCode)"'
                + 'required="true" rows = "'
        ...
        $(this).parent().children(".profile-detail  .profile-content-gender").each(function () {
            ...
            $(this).html('<select autofocus="1" onkeypress="userUpdateKeyPress($(this).parent().attr(\'id\'), , event.keyCode)" onchange="userUpdate($(this).parent().attr(\'id\'))">' +
        ...
        $(this).parent().children(".profile-detail  .profile-content-dob").each(function () {
            $(this).html('<input type="date"  required="true" onblur="userUpdate($(this).parent().attr(\'id\'))" value="' + $(this).text() + '">');
        });
    });
});
function userUpdateKeyPress(id, keycode) {
    var e = keycode;
    if (e == 13) {
        if ($("#" + id + " > input").length) {
            ...
            $.post("../userUpdate",
                {
                    ...
                    if (data == 0)
                        ...
                        if (id == 'dob')
                            $("#age").html(returnValue.getFullYear());
                        $("#" + id).html(returnValue);
                    }
